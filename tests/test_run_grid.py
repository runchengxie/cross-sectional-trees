...
def test_grid_supports_weighting_sweep(tmp_path, monkeypatch):
    ...
    output_csv = tmp_path / "grid_summary_weighting.csv"
    run_grid.main(
        [
            "--config",
            str(config_path),
    ...
    result = pd.read_csv(output_csv)
    assert len(result) == 2
    assert set(result["run_name"]) == {"demo_k2_bps10_wequal", "demo_k2_bps10_wsignal"}
    assert set(result["weighting"]) == {"equal", "signal"}
