...
def test_refresh_catalog_scans_manifest_backed_artifacts(tmp_path):
    ...
    raw_manifest = artifacts_root / "assets" / "rqdata" / "hk" / "daily" / "hk_all_daily_latest" / "manifest.yml"
    standardized_manifest = artifacts_root / "standardized" / "hk" / "daily_panel" / "hk_daily_panel" / "manifest.yml"
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        lineage = conn.execute(
            "SELECT relation, source_path FROM artifact_lineage WHERE artifact_id = ? ORDER BY relation",
            (str(standardized_manifest.resolve()),),
        ).fetchall()
        assert lineage == [
            ("source_asset_dir", str(raw_manifest.parent)),
...
def test_materialize_standardized_from_asset_dir_writes_partitioned_output(tmp_path):
    asset_dir = tmp_path / "artifacts" / "assets" / "rqdata" / "hk" / "daily" / "hk_all_daily_latest"
