...
def test_inspect_hk_asset_health_reports_stale_symbols_and_ffill_candidates(tmp_path, monkeypatch):
    ...
    assert field_map["pe_ratio_ttm"]["fresh_target_gap_on_target_date"] == 1
    assert field_map["pe_ratio_ttm"]["sample_oldest_ffill_symbols"] == []
    assert field_map["pe_ratio_ttm"]["sample_fresh_target_gap_symbols"] == [
        {
            "symbol": "00005.HK",
            "last_nonnull_date": "2026-03-30",
            "age_days": 1,
            "reference_context": "target_market_val_changed",
        }
    ]
    ...
    assert field_map["pb_ratio_ttm"]["ffill_age_days_max"] is None
    assert field_map["pb_ratio_ttm"]["fresh_target_gap_on_target_date"] == 1
    assert field_map["pb_ratio_ttm"]["sample_oldest_ffill_symbols"] == []
    assert field_map["pb_ratio_ttm"]["sample_fresh_target_gap_symbols"] == [
        {
            "symbol": "00005.HK",
            "last_nonnull_date": "2026-03-30",
            "age_days": 1,
...
def test_inspect_hk_asset_health_treats_fresh_valuation_rows_as_current_missing_not_stale(
    tmp_path, monkeypatch
):
    repo_root = tmp_path / "repo"
    asset_dir = repo_root / "artifacts" / "assets" / "rqdata" / "hk" / "valuation" / "valuation_fresh_gap_demo"
    data_dir = asset_dir / "data"
    data_dir.mkdir(parents=True)
    daily_asset_dir = repo_root / "artifacts" / "assets" / "rqdata" / "hk" / "daily" / "daily_fresh_gap_demo"
    daily_data_dir = daily_asset_dir / "data"
    daily_data_dir.mkdir(parents=True)
    monkeypatch.chdir(repo_root)
    ...
    pd.DataFrame(
        {
            "trade_date": ["20260328", "20260331"],
            "hk_total_market_val": [100.0, 100.0],
            "pe_ratio_ttm": [10.0, None],
        }
    ).to_parquet(data_dir / "00005.HK.parquet", index=False)
    pd.DataFrame(
    ...
    out_path = repo_root / "valuation_fresh_gap_health.json"
    args = SimpleNamespace(
        asset_dir=str(asset_dir),
        field=["pe_ratio_ttm"],
        date_column=None,
    ...
    field_map = {item["field"]: item for item in payload["field_coverage"]}
    assert field_map["pe_ratio_ttm"]["ffill_age_days_max"] is None
    assert field_map["pe_ratio_ttm"]["provider_ffill_age_days_max"] is None
    assert field_map["pe_ratio_ttm"]["fresh_target_gap_on_target_date"] == 1
    assert field_map["pe_ratio_ttm"]["sample_provider_like_ffill_symbols"] == []
    assert field_map["pe_ratio_ttm"]["sample_fresh_target_gap_symbols"] == [
        {
            ...
            "last_nonnull_date": "2026-03-28",
            "age_days": 3,
            "reference_context": "target_market_val_present",
        }
    ]
    assert payload["quality_checks"] == [
        {
