...
from cstree.research.hk_intraday_download import (
    _read_symbol_file,
    build_parser,
...
def test_download_hk_intraday_resume_validates_batch_metadata(tmp_path):
    ...
    download_hk_intraday_cache(_download_args(tmp_path, symbols_file=symbols_file, resume=False), rqdatac)
    assert rqdatac.calls == [["00005.XHKG", "00011.XHKG"]]
    assert (tmp_path / "hk_intraday_5m_20260409.parts" / "batch_0001.meta.json").exists()

    download_hk_intraday_cache(_download_args(tmp_path, symbols_file=symbols_file, resume=True), rqdatac)
    assert rqdatac.calls == [["00005.XHKG", "00011.XHKG"]]
