...
* `configs/experiments/variants/hk_selected__tr_close_execution_balanced_local.yml`：当前本地研究推荐入口，适合直接进入更合理的价格和执行近似口径；默认改用 `hk_selected_pit_research` 的 cap-weight benchmark 文件，避免 local daily snapshot 缺 `02800.HK` 时 benchmark / active 摘要中断。
