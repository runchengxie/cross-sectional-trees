...
3. 如果你想先确认“哪些 HK RQData 接口已经接了、哪些本地资产已核对、哪些别误用”，再看 [hk-rqdata-status.md](./hk-rqdata-status.md)。
4. 如果你已经确定后面大概率不会再续 RQData，再看 [hk-data-assets.md](./hk-data-assets.md#rqdata-权限失效前冻结清单) 把 current asset 和关键研究入口冻住。
5. 如果你已经拿到了 HK 分钟线，想继续看 `5m` 落盘快照、provider 边界和滑点校准，再看 [hk-intraday-assets.md](./hk-intraday-assets.md)。
...
| `hk-rqdata-status.md` | 哪些 HK RQData API 已接入、已有资产、哪些入口别误用 | 想补资产前先确认状态，或排查某条接口到底有没有打通 |
| `hk-data-assets.md` 的“RQData 权限失效前冻结清单” | RQData 权限快失效前，最后该冻结哪些资产和研究入口 | 你已经判断后面大概率离线运行，不再维护在线 refresh |
| `hk-intraday-assets.md` | HK `5m` 分钟线已经核对过哪些块、provider 边界在哪里、已经产出哪些滑点报告 | 准备继续补分钟线，或想把现有 `5m` 数据直接拿来校准执行假设 |
...
* `hk-data-assets.md` 负责资产准备和本地归档流程。
* `hk-rqdata-status.md` 负责记录 HK RQData API 的接入、资产和 probe 状态。
* `hk-intraday-assets.md` 负责记录 HK `5m` 分钟线操作快照、provider 边界和滑点校准产物。
