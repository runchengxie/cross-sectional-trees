...
如果新增十档盘口相关正式资产，建议采用新的 asset key，例如：
...
当前 `cross-sectional-trees` 尚未把这些 key 纳入主 pipeline，因此第一阶段只把它们作为共享数据契约预留项，不直接改变回测行为。
...
3. 扩展 `hk_current.json` 和 `dataset_registry.csv`，登记 `tick_depth_daily` 和未来的 `execution_cost_model`。
