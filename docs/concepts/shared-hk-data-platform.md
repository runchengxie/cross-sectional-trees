...
HK 数据层已经开始从策略仓中拆出：数据契约先固定，HK tick-depth 和 HK RQData
assets 的主要生产、检查、发布入口已经迁入 `market-data-platform`。

历史上 `cross-sectional-trees` 包含一套事实上的数据平台雏形：
...
* `cstree rqdata inspect-hk-*`、`cstree data catalog` 和 release 工具承担过资产检查、索引和分发职责；新的维护入口应优先使用 `marketdata rqdata hk-assets -- ...`。

因此适合采用分阶段拆分：

1. 先把 `artifacts_root` 外置成共享数据根目录。
2. 保持 `cross-sectional-trees` 只读消费 current contract 和 manifest-backed asset。
3. 通过 `market-data-platform` 维护十档盘口 raw / daily aggregate，并把日频聚合或成本模型派生物发布到同一个数据根目录。
4. 保留 `cstree rqdata ...` 作为兼容入口，直到下游调用全部切换到 `marketdata rqdata hk-assets -- ...`。
...
* 维护当前已公开的 `cstree rqdata ...` 入口作为兼容 shim，新的数据维护能力进入 `market-data-platform`。
...
6. 等下游调用全部切换后，将 `cstree rqdata ...` 降级为薄兼容入口或移除，并清理策略仓内重复的数据维护代码。
