...
这组汇总字段不只用于 `ic`，也常用于 `pearson_ic`、`train_ic`、`train_ic_raw`、`train_pearson_ic`、`cv_ic`、`cv_ic_raw` 和 `bucket_ic`：

* `n`：参与统计的有效交易日数或有效截面数。越小，结论越容易偶然波动。
* `mean`：这个指标在整个评估区间里的平均水平。
* `std`：这个指标在时间上的波动大小。越大，说明好坏时段差得越多。
* `ir`：这里就是 `mean / std`。它不是收益 Sharpe，而是“IC 的稳定度”。
* `t_stat`：平均值相对标准误有多大，可粗看“偏离 0 是否明显”。
...
* `cv_ic_raw`：交叉验证里未翻转方向前的原始 IC。主要用于方向判断，而不是直接当最终结果读。
...
* `permutation_test`：把训练标签打乱后重复评估得到的“噪声基线”。如果真实结果只比它好一点，就要警惕过拟合。
...
* `feature_importance_source`：重要性数值是按什么口径算出来的，不同模型之间不要直接横比绝对值大小。
