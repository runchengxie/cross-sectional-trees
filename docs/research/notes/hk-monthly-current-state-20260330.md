...
| `trial_008 + k15_bx25_be12` | 当前 ranker execution-layer challenger | 当前 monthly `no_ret` ranker 的主 challenger；适合继续做很小的 guardrail / exposure probe |
| `trial_008 + k15_bx20_be10` | aggressive comparator | 保留作更激进的实现对照；不作为默认 |
...
* `trial_008 + k15_bx25_be12` 把 ranker 路线推进到了更合理的 challenger 位置：walk-forward IC 证据比 `trial_016` 更干净，final OOS Sharpe / turnover / active IR 也有实现层增量；但它还没有强到替掉当前默认 PIT 候选。
...
2. 固定 `trial_008 + k15_bx25_be12` 为 ranker 主 challenger，只做一轮很小的 exposure / concentration guardrail probe；`trial_008 + k15_bx20_be10` 保留为 aggressiveness 对照。
...
* 很小资金 canary：要等新的样本和一次小的 guardrail probe。
...
当前 HK monthly 最合理的现行口径是：`M-PIT baseline` 保留为研究锚点，`M-PIT + no_ret + bx20 / be10` 保持默认 PIT candidate，`trial_008 + k15_bx25_be12` 升为 ranker 主 challenger，`M-provider rebalance-only` 保留为实现 comparator / shadow 候选；接下来优先等新样本和做小的 guardrail probe，而不是重开大网格。
