...
> 状态提示：本页属于专题分析（`active deep-dive`），用于把 quarterly 线当前几条容易混淆的 benchmark / challenger 放回同一解释框架。当前默认研究入口请先读 [hk-quarterly-current-state-20260329.md](./hk-quarterly-current-state-20260329.md)。
...
页面性质：`research-note`  
状态：`active deep-dive`，这页只做 quarterly 的 benchmark / challenger 解释层，不作为默认入口
最后核对时间：`2026-04-05`  
...
| 本页写法 | 含义 |
| --- | --- |
| benchmark | 用来做对照的基准路线 |
| challenger | 有局部亮点，仍需继续验证的候选路线 |
| structural challenger | 主要改善组合构造或持仓稳定性的结构候选 |
| sidecar | 独立回答一个问题的辅助路线 |
| final OOS | 最终留出期，不能参与训练或调参 |
...
* `raw-scale dedup + groupcap3` 更像结构 challenger；它的价值主要在组合修形和持仓稳定性，不负责重写信号故事。
* 纯 PIT 基本面路线值得保留，更适合当独立 benchmark / challenger，不适合直接替掉当前 hybrid 主线。
...
1. 如果它只是最近一段 OOS 变亮，先把它归到 challenger 观察，暂不升级主线。
2. 如果它主要改善换手、集中度或持仓稳定性，先把它归到结构 challenger，暂不说信号更强。
