...
本页解决什么：把当前 HK quarterly 研究线真正还在生效的默认主线、challenger、结构 probe 和数据 caveat 压成一页。  
...
读完你会得到什么：当前默认口径、主要 challenger、当前不再优先的方向，以及下一步最值得继续推进的少数动作。  
...
| 角色 | 当前定位 | 现在怎么用 |
| --- | --- | --- |
| `ranker h12_w16 + close + balanced execution` | 默认主线 | 仍是 quarterly 默认锚点；不是因为它完美，而是因为它在现有证据里最稳 |
| `reg_zscore h12_w16 + tr_close + balanced execution` | 第一 challenger | 保留为最值得跟踪的 challenger；最近 OOS 亮点不能直接覆盖前段弱证据 |
| `raw-scale dedup + groupcap3` | 结构 challenger | 主要回答组合修形和持仓稳定性问题，不直接等同于“信号更强” |
| `xgb_regressor + operating_margin` | 纯 PIT 基本面 sidecar | 作为 benchmark / challenger 线保留，不直接替掉当前 hybrid 主线 |
| `provider_dense` variant | 数据 / 实现 caveat 的配套变体 | 仅用于 coverage-sensitive probe 和 health gate，不是新的 quarterly 默认配置 |

## 2. 为什么现在是这套分工

* quarterly 的主问题仍然是 `regime shift / concept drift`，不是“模型名字还不够多”；这一点没有被后续 follow-up 推翻。
* `tr_close` 依然是路线相关结论，不是一刀切默认值：它对 ranker 主线证据不足，对 `reg_zscore` challenger 才是净正向加成。
* fixed-signal construction grid 已经说明 construction 值得做，也给出了 shortlist，但还没有给出足够干净的证据去替掉主线默认。
* 纯 PIT 基本面路线值得保留，但它回答的是“基本面本身有没有独立信息量”，不是“当前 hybrid 主线该立刻退场”。
...
* `raw-scale dedup + groupcap3` 更像结构 probe，不是把主线直接换掉的理由。
...
1. 冻结当前主线和 challenger 规格，把新的判断尽量留给后续前瞻样本，而不是继续扩模型 zoo。
...
1. 本页：先把默认主线、challenger 和数据 caveat 看对。
2. [`hk-quarterly-benchmark-and-interpretation-20260405.md`](./hk-quarterly-benchmark-and-interpretation-20260405.md)：理解主线、challenger、结构 probe 和纯基本面 sidecar 分别在回答什么。
3. [`hk-quarterly-holdings-analysis-20260329.md`](./hk-quarterly-holdings-analysis-20260329.md)：看结构 challenger 到底是在修组合还是改信号故事。
4. [`hk-quarterly-construction-grid-20260329.md`](./hk-quarterly-construction-grid-20260329.md)：看为什么 construction 结果现在更适合当 shortlist，而不是升级证据。
...
当前 HK quarterly 最合理的现行口径仍然是：`ranker h12_w16 + close + balanced execution` 保持默认主线，`reg_zscore h12_w16 + tr_close + balanced execution` 保持第一 challenger，`raw-scale dedup + groupcap3` 保持结构 probe，纯 PIT 基本面线继续做独立 sidecar；`provider_dense` 只作为 coverage-sensitive 变体保留，不升级成新的默认 baseline。
