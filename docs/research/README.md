...
| `notes/hk-monthly-ranker-ab-and-next-sweep-20260413.md` | 直接把 round 4 gated winner 参数换成 `xgb_ranker` 还不够；第一波 dated-asset ranker-native sweep 已找到 `trial_016`，同口径复跑稳定，fixed-signal construction grid 当前首选 `k15_bx25_be12`，它改善 final OOS Sharpe / turnover / active IR，但 walk-forward IC mean 仍偏弱；初步 attribution 显示 active 改善跨 benchmark 成立，但组合更集中且偏大盘 / 低波 / 质量暴露，因此只能升级为 monthly `no_ret` ranker execution-layer challenger | 否，当前作为 monthly ranker A/B、第一波 ranker-native sweep、construction grid 和初步归因结果页保留 |
