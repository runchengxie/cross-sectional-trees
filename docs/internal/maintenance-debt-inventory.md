...
生成时间：2026-05-06。统计命令：
`scripts/dev/run_tests.sh maintainability --json --limit 5`。统计范围为 `src/`、`scripts/`、
`tests/` 下排除 `__pycache__` 的 Python 文件；此快照只用于趋势判断，不代表实时仪表盘。

| 指标 | 当前值 | 说明 |
| --- | ---: | --- |
| Python 文件数 | 263 | 包括源码、脚本和测试 |
| Python 总行数 | 97,301 | 仅静态行数，用作趋势参考 |
| 超过 100 字符的行 | 1,222 | Markdown 不计入 |
| 超过 100 行的函数 | 189 | 包含测试函数 |
| 超过 250 行的函数 | 31 | 优先关注实现代码 |
| 超过 500 行的函数 | 2 | 需要单独跟踪的超大函数 |
| `C901` 文件级豁免 | 35 | 见 `pyproject.toml` |
| `rqdata public_api.__all__` | 49 | 稳定 facade 导出名数量 |

当前最大函数仍集中在 pipeline train-eval / runner、RQData mirror、backtest 和
RQData asset health。下一轮应继续按职责拆分，不把 import sorting、unused import
或 pyupgrade 清理混进行为保持重构。
...
* `mirror_financial.py` 的 PIT patch mirror 仍是复杂入口；manifest builder 已抽出，
  下一轮优先拆 patch context、batch error handling 和 audit/write state，再尝试撤销该新增 registry 项。
...
| 优先级 | 事项 | 当前建议 |
| --- | --- | --- |
| P1 | 继续拆大函数 / 大模块 | 优先 `train_eval_stage.py` 的 period / walk-forward context，`pipeline/runner.py` 的 setup / dataset load，`asset_health.py` 的 symbol scan / target-date field stats update，以及 RQData mirror 大函数 |
...
| `src/cstree/data_tools/rqdata_assets/mirror_financial.py` | PIT patch context、batch error handling、audit/write state | `uv run pytest tests/rqdata_assets/test_mirror_financial.py -q` |
...
| 行数 | 函数 | 文件 | 初步处理 |
| ---: | --- | --- | --- |
| 565 | `_run_train_eval_stage_impl` | `src/cstree/pipeline/train_eval_stage.py` | 已收口到 `TrainEvalRequest`，后续继续拆 eval / walk-forward context |
| 515 | `_run_legacy` | `src/cstree/pipeline/runner.py` | 已抽 service hooks / benchmark compare attachment；后续拆 setup、dataset load、outputs |
...
| 453 | `inspect_hk_asset_health` | `src/cstree/data_tools/rqdata_assets/asset_health.py` | 已抽 field coverage、payload/render/write；后续继续拆 symbol scan / target-date stats update |
| 453 | `patch_hk_pit_financials` | `src/cstree/data_tools/rqdata_assets/mirror_financial.py` | 已抽 resume 校验和 manifest/totals；后续继续拆 patch context、batch error handling |
