...
| `src/cstree/data_providers.py` | data consumption | Provider adapter still handles online RQData, local asset reads, cache, and normalization | `uv run pytest tests/test_data_providers_cache.py tests/test_fundamentals_providers.py -q` | Split local asset reads from provider runtime |
| `src/cstree/data_tools/build_hk_connect_universe.py` | universe tools | Universe builder still combines input resolution, liquidity filters, and dated output writing | `uv run pytest tests/test_universe_tools.py -q` | Extract filter and writer helpers |
