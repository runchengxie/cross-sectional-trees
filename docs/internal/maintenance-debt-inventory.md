...
| Python 总行数 | 97,091 | 仅静态行数，用作趋势参考 |
| 超过 100 字符的行 | 1,233 | Markdown 不计入 |
| 超过 100 行的函数 | 188 | 包含测试函数 |
| 超过 250 行的函数 | 31 | 优先关注实现代码 |
| 超过 500 行的函数 | 4 | 需要单独跟踪的超大函数 |
| `C901` 文件级豁免 | 35 | 见 `pyproject.toml` |
| `rqdata public_api.__all__` | 49 | 稳定 facade 导出名数量 |

当前最大函数仍集中在 RQData asset health / mirror、pipeline train-eval / runner 和
backtest。下一轮应继续按职责拆分，不把 import sorting、unused import 或 pyupgrade
清理混进行为保持重构。
...
| 525 | `patch_hk_pit_financials` | `src/cstree/data_tools/rqdata_assets/mirror_financial.py` | 下一轮优先拆 patch context、batch error handling 和 manifest builder |
