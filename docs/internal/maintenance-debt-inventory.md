...
| Python 总行数 | 99,402 | 仅静态行数，用作趋势参考 |
| 超过 100 字符的行 | 1,202 | Markdown 不计入 |
| 超过 100 行的函数 | 184 | 包含测试函数 |
| 超过 250 行的函数 | 27 | 优先关注实现代码 |
| 超过 500 行的函数 | 0 | 需要单独跟踪的超大函数 |
| `C901` 文件级豁免 | 25 | 见 `pyproject.toml` |
| `rqdata public_api.__all__` | 49 | 稳定 facade 导出名数量 |

当前已经没有超过 500 行的 Python 函数。最大函数仍集中在 RQData mirror、通用
mirror workflow、pipeline data / eval 和 release 编排；
...
本轮按 summary section、artifact writer 和 runtime settings 子职责继续拆 pipeline
编排层，保持 `summary.json` schema、artifact 路径、文件名、配置键、默认值和错误语义
不变。`>250` 行函数从 30 降到 27；`pipeline/output_summary_sections.py`、
`pipeline/output_artifacts.py` 和 `pipeline/config.py::resolve_runtime_settings`
不再出现在最大函数列表中。`output_artifacts.py` 和 `config.py` isolated C901 已清零，
并撤销对应 per-file ignore。
...
| 优先级 | 事项 | 当前建议 |
| --- | --- | --- |
| P1 | 继续拆大函数 / 大模块 | 当前先巩固 `>500` 清零结果；下一步优先 `mirror_financial.py` batch error/audit state、`mirror_workflow.py` resume state、`pipeline/panel_loader.py` source/date reader、`pipeline/eval.py` period scoring |
...
本表是 `pyproject.toml` 里当前 25 个文件级 `C901` 豁免的审计口径。owner area
...
| 319 | `_load_research_panel` | `src/cstree/pipeline/panel_loader.py` | 后续拆 source resolver、date window、panel reader |
