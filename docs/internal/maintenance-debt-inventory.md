...
| `rqdata_assets` package facade | `public_api.__all__` 48 个名字 | repo-local audit 未发现 private facade imports；新增测试确认 package/public API 不导出下划线 helper | `uv run pytest tests/test_cli_rqdata.py tests/rqdata_assets/test_request_groups.py -q` |
...
下一轮应优先继续拆 `asset_health` 的扫描/field aggregation、`runner` 的 setup/dataset load，
...
| 2,073 | `src/cstree/data_tools/rqdata_assets/asset_health.py` | RQData health inspection | 主函数已缩短；继续拆 scan、aggregation、report |
...
| 行数 | 函数 | 文件 | 初步处理 |
| ---: | --- | --- | --- |
| 749 | `test_cli_parses_rqdata_asset_commands` | `tests/test_cli_rqdata.py` | 测试函数也过大；后续可按命令族拆 |
| 581 | `inspect_hk_asset_health` | `src/cstree/data_tools/rqdata_assets/asset_health.py` | 已抽 field severity builder；后续继续拆扫描、汇总、report |
| 550 | `_run_train_eval_stage_impl` | `src/cstree/pipeline/train_eval_stage.py` | 已收口到 `TrainEvalRequest`，后续继续拆 eval / walk-forward context |
| 509 | `run` | `src/cstree/pipeline/runner.py` | 已抽 service hooks / benchmark compare attachment；后续拆 setup、dataset load、outputs |
...
| `src/cstree/data_tools/rqdata_assets/public_api.py` | 490 | 测试和程序化使用 facade | `__all__` 导出 48 个稳定名字；模块内部仍导入大量 private helpers |
...
导入 private helper；`public_api.__all__` 当前导出 48 个稳定名字、0 个下划线 helper。
