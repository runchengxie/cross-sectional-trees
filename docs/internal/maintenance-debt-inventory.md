...
| Python 总行数 | 99,834 | 仅静态行数，用作趋势参考 |
| 超过 100 字符的行 | 1,197 | Markdown 不计入 |
| 超过 100 行的函数 | 184 | 包含测试函数 |
| 超过 250 行的函数 | 24 | 优先关注实现代码 |
| 超过 500 行的函数 | 0 | 需要单独跟踪的超大函数 |
| `C901` 文件级豁免 | 23 | 见 `pyproject.toml` |
...
文件名、配置键、默认值、provider 语义和错误语义不变。`>250` 行函数从 30 降到 24；
...
`panel_loader.py` 和 `mirror_workflow.py` isolated C901 已清零，并撤销对应 per-file ignore。
...
* `mirror_financial.py` 的 PIT patch mirror 仍是复杂入口；manifest builder 和 request
  context 已抽出，下一轮优先拆 batch error handling 和 audit/write state，再尝试撤销该 registry 项。
...
| 优先级 | 事项 | 当前建议 |
| --- | --- | --- |
| P1 | 继续拆大函数 / 大模块 | 当前先巩固 `>500` 清零结果；下一步优先 `mirror_financial.py` batch error/audit state、`mirror_daily.py` fetch/resume/writer、`pipeline/eval.py` walk-forward window |
...
本表是 `pyproject.toml` 里当前 23 个文件级 `C901` 豁免的审计口径。owner area
...
| `src/cstree/data_tools/rqdata_assets/mirror_daily.py` | daily fetch policy、resume state、writer/finalize | `uv run pytest tests/rqdata_assets/test_mirror_daily.py -q` |
