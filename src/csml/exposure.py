...
def _build_active_exposure_summary_table(
    ...
    *,
    ...
    if not style_df.empty:
        for _, row in style_df[["rebalance_date", "entry_date"]].drop_duplicates().iterrows():
            periods.add((str(row["rebalance_date"]), row["entry_date"]))
    if not industry_df.empty:
        for _, row in industry_df[["rebalance_date", "entry_date"]].drop_duplicates().iterrows():
            periods.add((str(row["rebalance_date"]), row["entry_date"]))
...
def _price_history_tables(
    ...
    *,
    ...
    returns = price_table.pct_change().replace([np.inf, -np.inf], np.nan)
    momentum_tables = {
        window: price_table.pct_change(window).replace([np.inf, -np.inf], np.nan)
...
def _resolve_momentum_factor(
    ...
    *,
    ...
    values, used = _compose_from_columns(day, [(column, "identity") for column in _MOMENTUM_COLUMNS])
    ...
    for window, table in momentum_tables.items():
        ...
        if int(row.notna().sum()) == 0:
            continue
        derived_components.append(pd.Series(row.to_numpy(dtype=float), index=day.index, dtype=float))
...
def _resolve_low_vol_factor(
    ...
    *,
    ...
    direct_specs = [("low_vol", "identity"), ("low_volatility", "identity"), ("defensive", "identity")]
    inverse_specs = [(column, "neg") for column in _LOW_VOL_COLUMNS if column not in {"low_vol", "low_volatility", "defensive"}]
...
def _style_exposure_row(
    ...
    *,
    ...
    aligned_day = aligned_day.dropna(subset=["factor_z"])
    if aligned_day.empty:
        return {
            "rebalance_date": rebalance_date.strftime("%Y%m%d"),
            "entry_date": entry_date.strftime("%Y%m%d") if entry_date is not None else None,
    ...
    aligned_day = aligned_day.drop_duplicates(subset=["symbol"], keep="last")
    ...
    if np.isfinite(portfolio_long) and np.isfinite(portfolio_short):
        portfolio_net = float(portfolio_long - portfolio_short)
    elif np.isfinite(portfolio_long):
        portfolio_net = float(portfolio_long)
    elif np.isfinite(portfolio_short):
        portfolio_net = float(-portfolio_short)
    else:
        portfolio_net = np.nan
    ...
    if market_cap_col and market_cap_col in day.columns:
        ...
        if not cap_weights.empty:
            ...
            universe_cap_weight = _weighted_average(
                cap_weights["factor_z"],
                cap_weights[market_cap_col],
            )
    ...
    return {
        ...
        "active_net_vs_equal": (
            float(portfolio_net - universe_equal)
            if np.isfinite(portfolio_net) and np.isfinite(universe_equal)
            else np.nan
        ),
        "active_net_vs_cap": (
            float(portfolio_net - universe_cap_weight)
            if np.isfinite(portfolio_net) and np.isfinite(universe_cap_weight)
            else np.nan
        ),
    }
...
def _industry_exposure_rows(
    *,
    ...
    long_share = (
        long.groupby(industry_col)["weight"].sum() / long_total if long_total > 0 else pd.Series(dtype=float)
    )
    short_share = (
        short.groupby(industry_col)["weight"].sum() / short_total if short_total > 0 else pd.Series(dtype=float)
    )
    gross_share = (
        gross.groupby(industry_col)["weight"].sum() / gross_total if gross_total > 0 else pd.Series(dtype=float)
    )
    universe_equal = universe.groupby(industry_col)["symbol"].nunique()
    universe_equal = universe_equal / float(universe_equal.sum()) if not universe_equal.empty else universe_equal
...
def compute_backtest_exposure_analysis(
    ...
    *,
    ...
    positions = positions.dropna(subset=["rebalance_date_ts", "entry_date_ts", "symbol"])
    if positions.empty:
        return {
            "style": pd.DataFrame(),
            "style_summary": _empty_style_summary(),
    ...
    for rebalance_date, pos_day in positions.groupby("rebalance_date_ts", sort=True):
        ...
        size_values, size_meta = _resolve_size_factor(day, market_cap_col=market_cap_col)
        _update_factor_meta(factor_meta, "size", size_meta)
        style_rows.append(
            _style_exposure_row(
                "size",
                ...
                rebalance_date=rebalance_date,
                entry_date=entry_date,
                source_meta=size_meta,
            )
        )

        value_values, value_meta = _resolve_value_factor(day)
        _update_factor_meta(factor_meta, "value", value_meta)
        style_rows.append(
            _style_exposure_row(
                "value",
                ...
                rebalance_date=rebalance_date,
                entry_date=entry_date,
                source_meta=value_meta,
            )
        )

        quality_values, quality_meta = _resolve_quality_factor(day)
        _update_factor_meta(factor_meta, "quality", quality_meta)
        style_rows.append(
            _style_exposure_row(
                "quality",
                ...
                rebalance_date=rebalance_date,
                entry_date=entry_date,
                source_meta=quality_meta,
            )
        )

        momentum_values, momentum_meta = _resolve_momentum_factor(
            day,
            rebalance_date,
            momentum_tables=history["momentum_tables"],
        )
        _update_factor_meta(factor_meta, "momentum", momentum_meta)
        style_rows.append(
            _style_exposure_row(
                "momentum",
                ...
                rebalance_date=rebalance_date,
                entry_date=entry_date,
                source_meta=momentum_meta,
            )
        )

        low_vol_values, low_vol_meta = _resolve_low_vol_factor(
            day,
            rebalance_date,
            vol_tables=history["vol_tables"],
        )
        _update_factor_meta(factor_meta, "low_vol", low_vol_meta)
        style_rows.append(
            _style_exposure_row(
                "low_vol",
                ...
                rebalance_date=rebalance_date,
                entry_date=entry_date,
                source_meta=low_vol_meta,
            )
        )

        beta_values, beta_meta = _resolve_beta_factor(
            day,
            rebalance_date,
            beta_table=beta_table,
        )
        _update_factor_meta(factor_meta, "beta", beta_meta)
        style_rows.append(
            _style_exposure_row(
                "beta",
    ...
    if not style_df.empty:
        ...
        style_summary["latest_entry_date"] = str(latest_entry.iloc[0]) if not latest_entry.empty else None
        style_summary["latest"] = {
            str(row["factor"]): {
                "portfolio_long": float(row["portfolio_long"]) if pd.notna(row["portfolio_long"]) else np.nan,
                "portfolio_short": float(row["portfolio_short"]) if pd.notna(row["portfolio_short"]) else np.nan,
                "portfolio_net": float(row["portfolio_net"]) if pd.notna(row["portfolio_net"]) else np.nan,
                "universe_equal": float(row["universe_equal"]) if pd.notna(row["universe_equal"]) else np.nan,
                "universe_cap_weight": (
                    float(row["universe_cap_weight"]) if pd.notna(row["universe_cap_weight"]) else np.nan
                ),
                "active_net_vs_equal": (
                    float(row["active_net_vs_equal"]) if pd.notna(row["active_net_vs_equal"]) else np.nan
                ),
                "active_net_vs_cap": (
                    float(row["active_net_vs_cap"]) if pd.notna(row["active_net_vs_cap"]) else np.nan
                ),
                "source": row["source"],
                "source_columns": list(row["source_columns"]) if isinstance(row["source_columns"], list) else [],
