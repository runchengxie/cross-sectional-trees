...
class SideBpsCostModel:
    ...
    def cost(
        ...
        *,
        ...
        if side == "short":
            cost = (
                entry * float(self.short_entry_bps)
                + exit_ * float(self.short_exit_bps)
            ) / 10000.0
        ...
        return float(
            (
                entry * float(self.long_entry_bps)
                + exit_ * float(self.long_exit_bps)
            )
            / 10000.0
        )
    ...
    def cost(
        ...
        *,
        ...
        if (
            ...
            if valid.any():
                participation = trade_abs.loc[valid] * float(self.portfolio_value) / amounts.loc[valid]
                ...
                impact = float(self.impact_bps) * np.power(participation.to_numpy(dtype=float), float(self.power))
...
def build_slippage_model(slippage_cfg: Optional[Mapping]) -> SlippageModel:
...
def build_selection_constraints(
    constraints_cfg: Optional[Mapping],
...
def build_execution_model(
    ...
    *,
    ...
    default_price_col: str = "close",
    default_entry_price_col: Optional[str] = None,
    default_exit_price_col: Optional[str] = None,
) -> ExecutionModel:
    cost_cfg = None
    exit_cfg = None
    slippage_cfg = None
...
def describe_selection_constraints(constraints: SelectionConstraints) -> dict:
    return {
        "min_price": (
            float(constraints.min_price)
            if constraints.min_price is not None
            else None
        ),
        "min_amount": (
            float(constraints.min_amount)
