...
def build_backtest_tearsheet_html(
    *,
    ...
    if benchmark_label:
        subtitle_parts.insert(0, f"Benchmark: {benchmark_label}")

    charts = [
        _line_chart_svg(
            _cumulative_frame(strategy=strategy, benchmark=benchmark),
            title="Cumulative Returns vs Benchmark" if not benchmark.empty else "Cumulative Returns",
            value_kind="return",
        ),
        _drawdown_chart_svg(strategy, title="Underwater Plot"),
    ]
...
def _summarize_series(returns: pd.Series, *, periods_per_year: float) -> dict[str, Any]:
    ...
    ann_vol = vol * np.sqrt(periods_per_year) if _is_finite(vol) and _is_finite(periods_per_year) else np.nan
    sharpe = (
...
def _rolling_sharpe_frame(
    *,
    ...
    if strategy.empty or not _is_finite(periods_per_year) or periods_per_year <= 1:
        return pd.DataFrame()
    window = max(3, int(round(float(periods_per_year))))
...
def _line_chart_svg(frame: pd.DataFrame, *, title: str, value_kind: str) -> str:
    ...
    for idx, column in enumerate(frame.columns):
        ...
        legend.append(
            f'<span><i style="background:{color}"></i>{_escape(str(column))}</span>'
        )
...
def _monthly_returns_table(returns: pd.Series) -> str:
    ...
    rows.append("<thead><tr><th>Year</th>" + "".join(f"<th>{month}</th>" for month in _MONTH_LABELS) + "<th>Year</th></tr></thead>")
    rows.append("<tbody>")
    for year, row in pivot.iterrows():
        ...
        for month in range(1, 13):
            ...
            cells.append(
                f'<td class="{_heat_class(value)}">{_format_percent(value, digits=2, blank="-")}</td>'
            )
...
def _metrics_table(
    *,
    ...
    body = [
        "<table>",
        f"<thead><tr><th>Metric</th><th>{_escape(benchmark_header)}</th><th>Strategy</th></tr></thead>",
        "<tbody>",
    ]
    ...
    if active_stats:
        body.extend(
            [
                ...
                f"<tr><td>Active Total Return</td><td>-</td><td>{_format_metric(_metric(active_stats, 'active_total_return'), 'percent')}</td></tr>",
                f"<tr><td>Tracking Error</td><td>-</td><td>{_format_metric(_metric(active_stats, 'tracking_error'), 'percent')}</td></tr>",
                f"<tr><td>Information Ratio</td><td>-</td><td>{_format_metric(_metric(active_stats, 'information_ratio'), 'number')}</td></tr>",
                f"<tr><td>Beta</td><td>-</td><td>{_format_metric(_metric(active_stats, 'beta'), 'number')}</td></tr>",
                f"<tr><td>Alpha</td><td>-</td><td>{_format_metric(_metric(active_stats, 'alpha'), 'percent')}</td></tr>",
                f"<tr><td>Correlation</td><td>-</td><td>{_format_metric(_metric(active_stats, 'corr'), 'percent')}</td></tr>",
            ]
        )
...
def _eoy_returns_table(
    *,
    ...
    if strategy.empty:
        return "<p>No data.</p>"
    strategy_yearly = strategy.resample("YE").apply(lambda values: float((1.0 + values).prod() - 1.0))
    benchmark_yearly = (
    ...
    rows = [
        "<table>",
        f"<thead><tr><th>Year</th><th>{_escape(benchmark_header)}</th><th>Strategy</th><th>Won</th></tr></thead>",
        "<tbody>",
    ]
    for year in years:
        strategy_value = _year_value(strategy_yearly, year)
        benchmark_value = _year_value(benchmark_yearly, year)
        won = "+" if _is_finite(strategy_value) and _is_finite(benchmark_value) and strategy_value > benchmark_value else "-"
...
@media (max-width: 900px){body{margin:18px}#left,#right{float:none;width:100%;margin:0}h1 dt{display:block;margin-left:0;margin-top:4px}}
@media print{body{margin:0}.container{max-width:100%;margin:0}#left{width:58%;margin:0 2% 0 0}#right{width:40%}hr{margin:20px 0}}
""".strip()
